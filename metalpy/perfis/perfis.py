...
class PerfilEstrutural(SecaoGenerica):
    ...
    def definir_norma(self, norma):
        ...
        if norma not in NORMAS.keys():
            ...
            metodos = ('Ntrd_brt', 'Ncrd', 'Vrdx', 'Vrdy', 'Mrdx', 'Mrdy', 'Trd', 'verif_NM')
...
class PerfilILam(PerfilEstrutural):
    ...
    def __init__(self, nome, mat, und='mm', norma=None):
        ...
        self.d = float(self._dados_perfil['d'].iloc[0])
        self.bf = float(self._dados_perfil['bf'].iloc[0])
        self.tf = float(self._dados_perfil['tf'].iloc[0])
        self.tw = float(self._dados_perfil['tw'].iloc[0])
        self.kdes = float(self._dados_perfil['kdes'].iloc[0])

        self.r = self.kdes - self.tf
        self.h = self.d - 2 * self.tf
        self.dl = self.h - 2 * self.r

        self.esb_alma = float(self._dados_perfil['h/tw'].iloc[0])
        self.esb_mesa = float(self._dados_perfil['bf/2tf'].iloc[0])
    ...
    def _prop_geo(self):
        ...
        A = float(self._dados_perfil['A'].iloc[0])
        Ix = float(self._dados_perfil['Ix'].iloc[0]) * 1E6
        Iy = float(self._dados_perfil['Iy'].iloc[0]) * 1E6
        J = float(self._dados_perfil['J'].iloc[0]) * 1E3
        Wx = float(self._dados_perfil['Wx'].iloc[0]) * 1E3
        Wy = float(self._dados_perfil['Wy'].iloc[0]) * 1E3
        Zx = float(self._dados_perfil['Zx'].iloc[0]) * 1E3
        Zy = float(self._dados_perfil['Zy'].iloc[0]) * 1E3
        Cw = float(self._dados_perfil['Cw'].iloc[0]) * 1E9
...
class TuboRet(PerfilEstrutural):
    ...
    def __init__(self, nome, mat, und='mm', norma=None):
        ...
        self.h = float(self._dados_perfil['Ht'].iloc[0])
        self.b = float(self._dados_perfil['B'].iloc[0])
        self.esb_alma = float(self._dados_perfil['h/tdes'].iloc[0])
        self.esb_mesa = float(self._dados_perfil['b/tdes'].iloc[0])
        self.t = float(self._dados_perfil['tdes'].iloc[0])
        self.tw = self.t
        self.tf = self.t
        self.Wt = float(self._dados_perfil['Wt'].iloc[0]) * 1E3
...
class TuboCir(PerfilEstrutural):
    ...
    def __init__(self, nome, mat, und='mm', norma=None):
        ...
        self.D = float(self._dados_perfil['D'].iloc[0])
        self.t = float(self._dados_perfil['tdes'].iloc[0])
        self.esb = float(self._dados_perfil['D/t'].iloc[0])
