...
class AISC360(Norma):
    ...
    def Ntrd_brt(perfil, phi_s=0.9, data=False):
        ...
        warn('Item D1: A AISC360-16 recomenda que índice de esbeltez de barras tracionadas não supere o valor de 300')
    ...
    @staticmethod
    def Mrdx(perfil, Lb, Cb, phi_b=0.90, data=False):
        """
        ...
        if perfil.tipo == 'U LAMINADO':
            if not data:
                return AISC360._Mnx_LTB(perfil, Lb, Cb) * phi_b
            else:
                Mrdx_dados = namedtuple('Mrdx_dados', 'FLT')
                Mnx_LTB, dados_LTB = AISC360._Mnx_LTB(perfil, Lb, Cb, data)
                Mrdx = Mnx_LTB * phi_b
                return Mrdx, Mrdx_dados(dados_LTB)

        elif perfil.tipo in ('I LAMINADO', 'I SOLDADO'):
            if not data:
                return min(AISC360._Mnx_LTB(perfil, Lb, Cb), AISC360._Mnx_FLB(perfil)) * phi_b
            else:
                Mrdx_dados = namedtuple('Mrdx_dados', 'FLT FLM')
                Mnx_LTB, dados_LTB = AISC360._Mnx_LTB(perfil, Lb, Cb, data)
                Mnx_FLB, dados_FLB = AISC360._Mnx_FLB(perfil, data)
                Mrdx = min(Mnx_LTB, Mnx_FLB) * phi_b
                return Mrdx, Mrdx_dados(dados_LTB, dados_FLB)
            ...
            if not data:
                ...
                Mrdx_dados = namedtuple('Mrdx_dados', 'FLT FLM FLA')
                Mnx_LTB, dados_LTB = AISC360._Mnx_LTB(perfil, Lb, Cb, data)
                Mnx_FLB, dados_FLB = AISC360._Mnx_FLB(perfil, data)
                Mnx_WLB, dados_WLB = AISC360._Mnx_WLB(perfil, data)
                Mrdx = min(Mnx_LTB, Mnx_FLB, Mnx_WLB) * phi_b
                return Mrdx,  Mrdx_dados(dados_LTB, dados_FLB, dados_WLB)
            ...
            if not data:
                return min(AISC360._Mnx_FLB(perfil), AISC360._Mnx_WLB(perfil)) * phi_b
            else:
                Mrdx_dados = namedtuple('Mrdx_dados', 'FLM FLA')
                Mnx_FLB, dados_FLB = AISC360._Mnx_FLB(perfil, data)
                Mnx_WLB, dados_WLB = AISC360._Mnx_WLB(perfil, data)
                Mrdx = min(Mnx_FLB, Mnx_WLB) * phi_b
                return Mrdx, Mrdx_dados(dados_FLB, dados_WLB)

        elif perfil.tipo == 'TUBO CIR':
            if not data:
                return AISC360._Mn_Tubo(perfil) * phi_b
            else:
    ...
    @staticmethod
    def Mrdy(perfil, Lb, Cb, phi_b=0.90, data=False):
        """
        ...
        if perfil.tipo in ('I LAMINADO', 'I SOLDADO', 'U LAMINADO'):
            if not data:
                return AISC360._Mny_FLB(perfil) * phi_b
            else:
                Mrdy_dados = namedtuple('Mrdy_dados', 'FLM')
                Mny_FLB, dados_FLB = AISC360._Mny_FLB(perfil, data)
                Mrdy = Mny_FLB * phi_b
                return Mrdy, Mrdy_dados(dados_FLB)

        elif perfil.tipo in ('TUBO RET', 'CAIXAO') and perfil.Iy >= perfil.Ix:
            if not data:
                return min(AISC360._Mny_FLB(perfil), AISC360._Mny_LTB(perfil, Cb, Lb), AISC360._Mny_WLB(perfil))*phi_b
            else:
                Mrdy_dados = namedtuple('Mrdy_dados', 'FLT FLM FLA')
                Mny_LTB, dados_LTB = AISC360._Mny_LTB(perfil, Lb, Cb, data)
                Mny_FLB, dados_FLB = AISC360._Mny_FLB(perfil, data)
                Mny_WLB, dados_WLB = AISC360._Mny_WLB(perfil, data)
                Mrdy = min(Mny_WLB, Mny_FLB, Mny_LTB) * phi_b
                return Mrdy, Mrdy_dados(dados_LTB, dados_FLB, dados_WLB)

        elif perfil.tipo in ('TUBO RET', 'CAIXAO') and perfil.Iy < perfil.Ix:
            if not data:
                return min(AISC360._Mny_FLB(perfil), AISC360._Mny_WLB(perfil)) * phi_b
            else:
                Mrdy_dados = namedtuple('Mrdy_dados', 'FLM FLA')
                Mny_FLB, dados_FLB = AISC360._Mny_FLB(perfil, data)
                Mny_WLB, dados_WLB = AISC360._Mny_WLB(perfil, data)
                Mrdy = min(Mny_WLB, Mny_WLB) * phi_b
                return Mrdy, Mrdy_dados(dados_FLB, dados_WLB)
        else:
            NotImplementedError('Método não implementado para perfis do tipo {}'.format(perfil.tipo))
    ...
    @staticmethod
    def verif_NM(perfil, Nsd, Msdx, Msdy, phi=0.9, Lbx=None, Lby=None, Cb=1, klx=None, kly=None, klz=None, data=False):

        if Nsd > 0:
            Nrd = AISC360.Ntrd_brt(perfil, phi_s=phi, data=data)

        else:
            if (klx is not None) and (kly is not None) and (klz is not None):
                Nrd = AISC360.Ncrd(perfil, klx, kly, klz, phi_c=phi, data=data)
            else:
                raise ValueError('Para barras comprimidas devem ser fornecidos os comprimentos de flambagem klx, kly e klz')

        Mrdx = AISC360.Mrdx(perfil, phi_b = phi, Lb=Lbx, Cb=Cb, data=data)
        Mrdy = AISC360.Mrdy(perfil, phi_b= phi, Lb=Lby, Cb=Cb, data=data)

        if not data:
            if abs(Nsd/Nrd) >= 0.2:
                return abs(Nsd/Nrd) + (8/9) * (Msdx/Mrdx + Msdy/Mrdy)

            else:
                return abs(Nsd/(2*Nrd)) + (Msdx/Mrdx + Msdy/Mrdy)

        else:
            if abs(Nsd[0]/Nrd[0]) >= 0.2:
                return abs(Nsd[0]/Nrd[0]) + (8/9) * (Msdx[0]/Mrdx[0] + Msdy[0]/Mrdy[0]), Nsd, Mrdx, Mrdy

            else:
                return abs(Nsd[0]/(2*Nrd[0])) + (Msdx[0]/Mrdx[0] + Msdy[0]/Mrdy[0]), Nsd, Mrdx, Mrdy