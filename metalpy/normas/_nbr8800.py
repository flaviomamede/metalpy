...
class NBR8800(Norma):
    ...
    def _kc(esb):
        ...
        """ EU esb = h/tw ou d/tw ou b/tw"""
        """ EU cuidado: não usou perfil.esb """
    ...
    # Métodos auxiliares para a determinação do fator Qs de acordo com a seção F.2 (elementos comprimidos) do anexo F da NBR8800
    @staticmethod
    def _Qs_g3(perfil):
    ...
    def Ncrd(perfil, klx, kly, klz, gama_a1=1.1, data=False):
        ...
        Ncrd_dados = namedtuple('Ncrd_dados', 'Ncrk A fy Ne ier Chi Q')
        ...
        return Ncrd if not data else (Ncrd, Ncrd_dados(Ncrk, perfil.A, perfil.mat.fy, Ne, ier2, Chi2, Q))
    ...
    def Mrdx(perfil, Lb=None, gama_a1=1.1, Cb=1, data=False):
        ...
        if perfil.tipo in ('I LAMINADO', 'U LAMINADO', 'U SOLDADO'):
            ...
            if not data:
                ...
                return Mrdx, Mrdx_dados(dados_FLT, dados_FLM, dados_FLA)
            ...
            if not data:
                ...
                return Mrdx, Mrdx_dados(dados_FLM, dados_FLA)
            ...
            if not data:
                ...
                return Mrdx, Mrdx_dados(dados_FLT, dados_FLM)
            ...
            if perfil.esb_alma < elr_alma:
                ...
                if perfil.bissimetrico:
                    ...
                    if not data:
                        ...
                        Mrdx_dados = namedtuple('Mrdx_dados', 'EMT FLT FLM')
                        ...
                        return Mrdx, Mrdx_dados(dados_EMT, dados_FLT, dados_FLM)
                    ...
                    if not data:
                        ...
                        return (Mrdx1, Mrdx_dados(dados_EMT1, dados_FLT1, dados_FLM1)),\
                               (Mrdx2, Mrdx_dados(dados_EMT2, dados_EMT2, dados_FLM2))
        else:
            raise NotImplementedError('Mrdx não implementado para perfis do tipo {}'.format(perfil.tipo))
    ...
    @staticmethod
    def Mrdy(perfil, Lb=None, gama_a1=1.1, Cb=1, data=False):
        """
        ...
        if perfil.tipo in 'U LAMINADO':
            if not data:
                ...
                return Mrdy, Mrdy_dados(FLM_dados, FLA_dados)

        if perfil.tipo in ('I LAMINADO', 'I SOLDADO') and perfil.bissimetrico:
            if not data:
                ...
                Mny_FLM, FLM_dados = NBR8800._Mny_FLM(perfil, data)
                Mrdy = min(Mny_FLM, Mny_FLM, 1.5 * perfil.Mry) / gama_a1
                return Mrdy, Mrdy_dados(FLM_dados)
            ...
            if not data:
                ...
                return Mrdy, Mrdy_dados(FLT_dados, FLM_dados, FLA_dados)
    ...
    # -> Método para verificação dos esforços combinados
    # --------------------------------------------------

    @staticmethod
    def verif_NM(perfil, Nsd, Msdx, Msdy, gama_a1=1.1, Lbx=None, Lby=None, Cb=1, klx=None, kly=None, klz=None, data=False):

        if Nsd > 0:
            Nrd = NBR8800.Ntrd_brt(perfil, gama_a1=gama_a1, data=data)

        else:
            if (klx is not None) and (kly is not None) and (klz is not None):
                Nrd = NBR8800.Ncrd(perfil, klx, kly, klz, data=data)
            else:
                raise ValueError('Para barras comprimidas devem ser fornecidos os comprimentos de flambagem klx, kly e klz')

        Mrdx = NBR8800.Mrdx(perfil, gama_a1=gama_a1, Lb=Lbx, Cb=Cb, data=data)
        Mrdy = NBR8800.Mrdy(perfil, gama_a1=gama_a1, Lb=Lby, Cb=Cb, data=data)
